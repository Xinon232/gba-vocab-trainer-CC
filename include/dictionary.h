...
inline int slot(const unsigned char* r,unsigned seq){if(std::memcmp(r+204,"OK01",4))return 0;if(std::memcmp(r,"ADD1",4)||number(r+4)!=seq||crc(r,200)!=number(r+200)||!row_ok(reinterpret_cast<const char*>(r+8)))return -1;unsigned n=std::strlen(reinterpret_cast<const char*>(r+8));while(n<192)if(r[8+n++])return -1;return 1;}
}
class Dictionary {
public:
 ...
 Dictionary(DictionarySource* source,int index):source_(source),index_(index){
  if(!source_||!source_->read(index_,0,h_,160)||std::memcmp(h_,"GVDIDX01",8)||dict_format::crc(h_,156)!=num(156))return;
