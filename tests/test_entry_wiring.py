...
assert 'in.select_pressed = false;' in s
assert 'if (action == EntryShortcuts::Action::menu)' in s
assert s.index('in.select_pressed = false;') < s.index('state.update(g_vocab_file, in);')
