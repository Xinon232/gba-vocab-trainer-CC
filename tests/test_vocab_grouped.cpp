...
static int test_utf8_russian_and_logical_arabic_preserved()
{
    printf("[5] UTF-8 Russian and logical Arabic preserved\n");
    ...
    if (!vocab_show(vf, data, (int)strlen(data), 1, lb) || strcmp(lb.b, "ماء") != 0) {
        printf("    FAIL: Arabic got '%s' / '%s'\n", lb.a, lb.b);
        return 1;
    }
    if (!vocab_show(vf, data, (int)strlen(data), 2, lb) || strcmp(lb.b, "كتب") != 0) {
...
int main()
{
    ...
    rc |= test_utf8_russian_and_logical_arabic_preserved();
