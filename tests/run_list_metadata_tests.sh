...
for name in test_list_metadata test_list_metadata_faults test_list_settings test_list_settings_faults; do
 "$CXX" "${FLAGS[@]}" src/writer_core.cpp src/state.cpp src/vocab.cpp src/vocab_file_io.cpp tests/host_fatfs.cpp "tests/$name.cpp" -o "$BUILD/$name"
done
"$BUILD/test_list_metadata"
"$BUILD/test_list_settings"
"$BUILD/test_list_settings_faults"
