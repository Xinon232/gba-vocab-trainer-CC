"""Package a native standalone .dict builder with bundled Python/Tk."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    subprocess.run([sys.executable, '-m', 'PyInstaller', *extra, '--noconfirm', '--clean', '--windowed', '--onedir', '--name', 'gbavocab-builder', '--distpath', str(args.output), '--workpath', str(args.output / 'work'), '--specpath', str(args.output), str(ROOT / 'builder/app.py')], check=True)
    ...
    assert report['roundtrip'] and report['packaged'] and report['entries'] == 40010
