...
The app is built with Butano and targets SuperFW / Supercard SD-style setups. TXT files are compatible with dict.cc-style vocab-trainer exports. The home screen reads `gbavocab v1.6.0-pre.3`. V1.2 retains both cards and the existing green/red feedback while A/B is held, then for 24 frames after release (about 0.4 seconds, matching GBAWriter's initial solo-A repeat delay). Holding does not consume this delay. Grading remains on press; only advancement waits longer.
...
Version **1.6.0-pre.3** adds dictionary New/Edit/Delete through both routes and per-list saved mode/dictionary choice. The Windows/Linux builder retains **Import TXT / TSV / ZIP** and paginated **All / Flagged** review, reads v1/v2, and writes editable v2 dictionaries. Fonts and the 191-byte pair limit are unchanged. A dict.cc DE-ES header suggests German-Spanish and de/es (editable). Source comments/license and extra metadata remain visible, not vocabulary output. Meaningful `{gender}`, `[usage]`, `(qualifiers)`, accents and punctuation stay exactly as imported; no alternate cleaned search keys are added.
...
Hold **Start+Up/Down** to browse, **Start+A** to select, **Start+L** to change direction, **Start+R** for the chooser, **Start+Left** to edit, **Start+Right** to delete, and **Start+B** to cancel. Home lookup chooses a destination TXT through the existing Save/Discard/Cancel guard. Add from dictionary prefills a new draft in the current list. Canonical list columns do not change with search direction.

List language codes, remembered learning mode and preferred matching dictionary filename live in matching-basename `.sav`, not TXT: `Spanish.txt` uses `Spanish.sav`. Words and learning progress remain in TXT. First use without metadata asks for the pair; manual save persists all settings. Missing/legacy mode defaults to Alternate. A matching saved dictionary opens automatically; a sole match is remembered, multiple matches ask once, and an unavailable saved file prompts again. Start+R remains an explicit chooser. Settings-only saves do not rewrite TXT. Copy/rename the pair together. Valid legacy `# gbavocab: front=en; back=de` footers migrate only after matching SAV write, sync and readback verification. Corrupt/conflicting metadata blocks saving. Orphan SAV and SAV.tmp names are reserved when creating new lists; neither is silently adopted or deleted. See [metadata format](docs/list-metadata-format.md).

**Manage dictionary words:** from either main-menu lookup or Entry editor > Add from dictionary, press **Start+Select** for New, **Start+Left** for Edit selected, or **Start+Right** for Delete selected. Delete defaults to No; Left/Right chooses No/Yes, A confirms and B cancels. The entire unfinished outer list draft survives nested success, failure and cancellation. Enter the displayed canonical languages; **Start+A** advances then saves immediately to the same .dict file. **Start+B** goes back/cancels. Checksummed, two-phase committed append slots leave the base and previous slots unchanged; incomplete tails consume a slot and are skipped on retry. Exact duplicate addition retries do not append twice. New/Edit/Delete share 512 physical slots. Stable identities keep duplicate senses separate and retries do not repeat completed mutations. V1 Edit/Delete requires PC Open .dict / Save As to create v2; v1 reading/addition remains supported. At 512 physical slots, compact with PC Open .dict / Save As. Back up before replacing files. Unpublished legacy dictionary `.sav` files are unsupported and never automatically removed; keep copies for manual recovery.
...
- Persisted mutations use the existing validated TXT replacement/recovery transaction. Manual saves use the existing grouped CRLF save format. Matching-basename `.sav` stores the list pair, mode and preferred dictionary; transient recovery files are retained when failure requires them. Load or create an SD-backed TXT before opening the editor; there is no demo-list drafting fallback.
