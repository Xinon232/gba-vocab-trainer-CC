...
The menu order is Add entry, Edit entry, Delete entry, Add from dictionary. Up/Down selects; A opens an action; B returns to learning. All confirmations are RAM-only. Add from dictionary opens matching ROM dictionaries and prefills a NEW draft, never overwriting the captured entry.
...
Manual saving uses the inherited grouped CRLF serializer and box-separator normalization. An optional final language-pair footer is written once. Its exact grammar, compatibility tradeoff, dictionary controls and PC builder instructions are in the full manual. No permanent `.sav`, settings or sidecar files are written.
