...
## Confirmation, dictionary lookup and manual saving

The menu order is Add entry, Edit entry, Delete entry, Add from dictionary. Up/Down selects; A opens an action; B returns to learning. All confirmations are RAM-only. Add from dictionary opens matching external .dict dictionaries and prefills a NEW draft, never overwriting the captured entry.
...
Confirmations apply immediately to the active list in RAM with zero TXT rewriting. Learning Start saves all pending entry changes, pair metadata and box progress; choosing Save before switching lists does likewise. No per-keystroke save occurs and canceling a draft never applies it.
...
Manual saving uses the inherited grouped CRLF serializer and box-separator normalization. Matching-basename .sav stores front/back language codes; words and progress remain in TXT. A valid legacy footer migrates only after durable SAV readback. Direct mutation also enforces blocked/verified metadata prerequisites. NEW LIST reserves orphan .sav and .sav.tmp names. External .dict files contain the base indexes and their own committed additions. See the full manual and dictionary-format.md.
